...
#			die();
	...
	private function get_picklist_ul($field) {
		...
		foreach($field['picklistValues'] as $value) {
			...
			$str .= '<li style="margin:0; padding:0;" data-default="'.floatval(!empty($value->defaultValue)).'" data-value="'.js_escape($value->value).'" data-label="'.js_escape($value->label).'">'.js_escape($value->label).$default.'</li>';
		}
		$str .= '</ul>';
		return $str;
	}
	...
	<script type='text/javascript'>
		...
		function SFDisableChoices($scope, enable) {

			// If it's one-time, no disabling.
			if(
			   jQuery('input[name=salesforce_map_type]').val() === 'once' ||
			   jQuery('#salesforce_map_enabled').attr('checked') === false
			) {
				enable = true;
			}
			...
			if(!$scope) {
				...
				$scope = jQuery('#field_'+field['id']);
			}

			if(enable) {
				// Enable modifying the choices.
				jQuery('#field_choices input', $scope).attr('disabled', false);

				// Show sorting, add, and remove choices images
				jQuery('.add_field_choice, .delete_field_choice, .field-choice-handle, .choices_setting input.button', $scope).show();
				jQuery('#field_choice_values_enabled', $scope).parent('div').show();
			} else {
				// Disable modifying the choices.
				jQuery('#field_choices input', $scope).attr('disabled', true);

				// Hide sorting, add, and remove choices images
				jQuery('.add_field_choice, .delete_field_choice, .field-choice-handle,  .choices_setting input.button', $scope).hide();
				jQuery('#field_choice_values_enabled', $scope).parent('div').hide();
			}
		}
